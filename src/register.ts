...
import { APIS, WSS, REGISTER, CHAIN_VERSIONS, BASIS, Network, ZIL_HASH } from './constants'
...
export type ContractState = {
  ...
  launcher_address_called: boolean
}
...
/***************************************************/
/* Register Class to interact with Register.scilla */
/***************************************************/
export class Register {
  ...
  constructor(readonly network: Network, walletProviderOrKey?: WalletProvider | string, options?: Options) {
    ...
    this.contractAddress = REGISTER[network]
  ...
  /**
   * Stops watching the Register contract state.
   */
  ...
  /**
   * Gets the latest Register app state.
   */
  ...
  private async updateAppState(): Promise<void> {
    ...
    requests.push({ id: '1', method: 'GetSmartContractSubState', params: [address, 'register', []], jsonrpc: '2.0' })
    requests.push({ id: '2', method: 'GetSmartContractSubState', params: [address, 'tokens', []], jsonrpc: '2.0' })
    requests.push({ id: '3', method: 'GetSmartContractSubState', params: [address, 'launcher_address', []], jsonrpc: '2.0' })
    requests.push({ id: '4', method: 'GetSmartContractSubState', params: [address, 'launcher_address_called', []], jsonrpc: '2.0' })
  ...
  /**************************************************/
  /**************************************************/
  /* Lock Register address in Oracle*/
  public async SetRegisterOracle(): Promise<ObservedTx> {
    // Check logged in
    this.checkAppLoadedWithUser()

    console.log('sending set SetRegisterOracle txn..')
    const SetRegisterOracleTxn = await this.callContract(
      this.contract,
      'SetRegisterOracle',
      [],
      {
    ...
    if (SetRegisterOracleTxn.isRejected()) {
      throw new Error('Submitted transaction was rejected.')
    } else {
      console.log(SetRegisterOracleTxn.id!)
    }

    const observeTxn = {
      hash: SetRegisterOracleTxn.id!,
  ...
  /* Influencer can request to register her link */
  public async RegisterLink(link: string): Promise<ObservedTx> {
    // Check logged in
    this.checkAppLoadedWithUser()

    console.log('sending set RegisterLinkTxn txn..')
    const RegisterLinkTxn = await this.callContract(
      this.contract,
      'RegisterLink',
      [
        {
    ...
    if (RegisterLinkTxn.isRejected()) {
      throw new Error('Submitted transaction was rejected.')
    } else {
      console.log(RegisterLinkTxn.id!)
    }

    const observeTxn = {
      hash: RegisterLinkTxn.id!,
      deadline: this.deadlineBlock(),
    }
    await this.observeTx(observeTxn)
  ...
  /* Influencer request token creation */
  public async RequestToken(name: string, symbol: string, decimals: number, supply: number): Promise<ObservedTx> {
    // Check logged in
    this.checkAppLoadedWithUser()

    console.log('sending set RequestToken txn..')
    const RequestTokenTxn = await this.callContract(
      this.contract,
      'RequestToken',
      [
        {
    ...
    if (RequestTokenTxn.isRejected()) {
      throw new Error('Submitted transaction was rejected.')
    } else {
      console.log(RequestTokenTxn.id!)
    }

    const observeTxn = {
      hash: RequestTokenTxn.id!,
      deadline: this.deadlineBlock(),
    }
    await this.observeTx(observeTxn)
